...
Commands: `name := <expr>`, `vars`, `clear [name]`, `∂`/`diff`, `∫`/`int`, `simplify`, `expand`, `cancel`, `together`, `nf`/`normal`, `∑`/`sum`, `∏`/`product`, `dsolve`, `rsolve`, `limit`/`limleft`/`limright`, `apart`, `help`.
...
| `rsolve(eq)` | Const-coeff recurrence in `y(n)`, `y(n+1)`, … (char poly; poly / `q^n` forcing); first-order `y(n+1)=a(n)y(n)+b(n)` via `product`/`sum` |
| `rsolve(eq, y(n))` | Specify the sequence and index |
| `rsolve(eq, y0, y1[, …])` | ICs `y(0)=y0`, `y(1)=y1`, … |
| `rsolve(A)` | Linear system **Y(n+1) = A Y(n)** via Jordan `A^n` |
| `rsolve(A, Y0)` | Homogeneous system with Y(0)=Y0 |
| `rsolve(A, g)` | **Y(n+1) = A Y + g(n)** when `g` depends on `n` |
| `rsolve(A, g, n)` | Constant forcing `g` (independent `n`) |
| `rsolve(A, g, Y0)` | Nonhomogeneous with Y(0)=Y0 |
...
lake exe taschenrechner 'sum(binom(n,k)*x^k, k, 0, n)' # → (1+x)^n
lake exe taschenrechner 'sum(binom(n,k)^2, k, 0, n)'   # → C(2n,n)
...
lake exe taschenrechner "dsolve(x^2*y'' + x*yp - y = 0)"  # Cauchy–Euler → y = C1·x + C2/x
...
lake exe taschenrechner 'rsolve(y(n+1)-2*y(n)=0)'      # → y = C·2^n
lake exe taschenrechner 'rsolve(y(n+1)-y(n)=1)'        # → y = n + C
lake exe taschenrechner 'rsolve(y(n+1)-(n+1)*y(n)=0)'  # → y = C·n!
lake exe taschenrechner 'rsolve(y(n+2)-y(n+1)-y(n)=0, 0, 1)'  # Fibonacci
lake exe taschenrechner 'rsolve([0,1;1,1], [0;1])'     # same via A^n
lake exe taschenrechner 'rsolve([2,0;0,3],[1;0],n)'    # Y(n+1)=AY+g
...
lake exe taschenrechner 'dsolve([1,0;0,2],[1;0],x)'    # Y'=AY+g → y1 = C1·exp(x)−1, y2 = C2·exp(2x)
